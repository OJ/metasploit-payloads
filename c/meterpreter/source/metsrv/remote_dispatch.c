...
DWORD load_extension(HMODULE hLibrary, BOOL bLibLoadedReflectivly, Remote* remote, Packet* response, Command* pFirstCommand)
{
	...
	if (pExtension)
	{
		...
		if (bLibLoadedReflectivly)
		{
			...
			pExtension->getname = (PSRVGETNAME)GetProcAddressByOrdinalR(pExtension->library, EXPORT_GETEXTENSIONNAME);
		...
		else
		{
			...
			pExtension->getname = (PSRVGETNAME)GetProcAddressByOrdinal(pExtension->library, EXPORT_GETEXTENSIONNAME);
